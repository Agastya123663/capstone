...
// creating a variable for touching
var touching = [];
...
function draw() {
  ...
  drawSprites();
    ...
    if(mousePressedOver(playButton) || touching.length > 0){
      gameState = play
      playButton.visible = false;
      touching = [];
    }
    ...
    // increasing the timer if the player scores a goal
    ...
    if(keyWentDown("UP_ARROW")|| touching.length > 0){
      ball.velocityY = -30
      touching = [];
    }

    if(keyWentDown("DOWN_ARROW")||touching.length > 0){
      ball.velocityY = 30
      touching = [];
    }

    if(keyWentDown("RIGHT_ARROW")||touching.length > 0){
      ball.velocityX = 30
      touching = [];
     }

    if(keyWentDown("LEFT_ARROW")||touching.length > 0){
      ball.velocityX = -30
      touching = [];
     } 
